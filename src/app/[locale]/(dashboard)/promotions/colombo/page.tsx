...
export default function PromotionsColomboPage() {
  const colomboPromos = promotions.filter(p => p.branchId === 'colombo');
  const weekly = colomboPromos.filter(p => p.type === 'weekly');
  const monthly = colomboPromos.filter(p => p.type === 'monthly');

  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold bg-gradient-to-r from-orange-700 to-yellow-400 bg-clip-text text-transparent">Colombo Promotions</h1>
      <div>
