...
export default function StaffAllPage() {
  ...
  let staff = users.filter(u => u.role === 'manager' || u.role === 'staff');
  if (selectedBranchView !== 'all') {
    staff = staff.filter(u => u.branchId === selectedBranchView);
  }
  return (
      ...
      {staff.map(staff => (
        <Card key={staff.id} className="group bg-gradient-to-br from-yellow-100 via-orange-50 to-yellow-200 border-0 shadow-xl hover:scale-105 transition-transform">
          ...
          <CardContent className="flex flex-col items-center gap-1 text-sm text-gray-700">
            <div className="font-medium">{branches.find(b => b.id === staff.branchId)?.name || 'All Branches'}</div>
